...
async def set_hm(key:str, val:dict, ttl_in_sec: int = 3600):
    try:
        logging.info("[REDIS_UTIL][data received for set] {} {}".format(type(key), type(val)))
        redis_client.hmset(key,val)
        redis_client.expire(key, ttl_in_sec)

        print(" DATA SET ")
        res = await get_hm(key)
        print(" DATA GOTOO : ", str(res))
...
async def get_hm(key:str):
    try:
        ...
        print(" adat in bytes ",data_in_bytes)
        if data_in_bytes :
...
async def set_str(key: str, val:str, ttl_in_sec:int = 3600):
    try:
        ...
        redis_client.expire(ttl_in_sec)
    except Exception as ex :
...
async def is_exists(key : str):
    try:
        return redis_client.hexists(key)
